    ...
    def drive_robot_to_target_pose(self, target_pose=None, tracking_error_threshold=0.003, num_iter_max=100):
        ...
        while error > tracking_error_threshold and num_iters < num_iter_max:
            ...
            if no_improve_steps > 10:
                ...
                self.set_robot_joint_positions(self.some_safe_joint_angles + np.random.normal(0, 0.1, (7)))
                # env.step() 多次
                for _ in range(50):
                    self.env.sim.forward()
                    self.env.sim.step()
...
class SourceEnvWrapper:
    ...
    def generate_image(self, num_robot_poses=5, num_cam_poses_per_robot_pose=10, save_paired_images_folder_path="paired_images", reference_joint_angles_path=None, reference_ee_states_path=None, reference_gripper_states_path=None, robot_dataset=None, start_id=0):
        ...
        if self.fixed_cam_positions is None:
            # 生成全局坐标系下的固定相机位姿
            raw_pos, raw_quat = sample_half_hemisphere(num_cam_poses_per_robot_pose)
            self.fixed_cam_positions = raw_pos
            self.fixed_cam_quaternions = raw_quat
        
        cam_positions = self.fixed_cam_positions
        cam_quaternions = self.fixed_cam_quaternions
        ...
        if reference_gripper_states_path is not None:
            gripper_states = np.loadtxt(reference_gripper_states_path)
        ...
        if robot_dataset is not None:
            ...
            reference_gripper_states_path = robot_dataset_info["gripper_states_path"]
            joint_angles = np.loadtxt(reference_joint_angles_path)
            ee_states = np.loadtxt(reference_ee_states_path)
            gripper_states = np.loadtxt(reference_gripper_states_path)
        ...
        for pose_index in range(start_id, min(start_id+1000, num_robot_poses)):
            ...
            counter = 0
            os.makedirs(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_rgb", str(pose_index)), exist_ok=True)
            os.makedirs(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_rgb_brightness_augmented", str(pose_index)), exist_ok=True)
            os.makedirs(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_mask", str(pose_index)), exist_ok=True)
            attempt_counter = 0
            ...
            while not both_reached:
                attempt_counter += 1
                print(f"[INFO] 第{attempt_counter}次尝试")
                if attempt_counter > 10:
                    break

                gripper_open = gripper_convert(gripper_states[pose_index], "maskvit")
            ...
            for i, (pos, quat) in enumerate(zip(cam_positions, cam_quaternions)):
                ...
                print(f"\033[32m[NOTICE] 已完成第{pose_index}个pose的生成\033[0m")
                cv2.imwrite(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_rgb", f"{pose_index}/{counter}.jpg"), cv2.cvtColor(source_robot_img, cv2.COLOR_RGB2BGR))
                cv2.imwrite(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_rgb_brightness_augmented", f"{pose_index}/{counter}.jpg"), cv2.cvtColor(source_robot_img_brightness_augmented, cv2.COLOR_RGB2BGR))
                cv2.imwrite(os.path.join(save_paired_images_folder_path, f"{source_name.lower()}_mask", f"{pose_index}/{counter}.jpg"), source_robot_seg_img * 255)
                counter += 1
