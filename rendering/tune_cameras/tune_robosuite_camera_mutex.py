...
if __name__ == "__main__":
    ...
    joint_angles = np.loadtxt("/home/lawrence/xembody_followup/mutex_dataset/joint_states.txt")
    ee_poses = np.loadtxt("/home/lawrence/xembody_followup/mutex_dataset/ee_states.txt")
    ...
    for k in range(len(joint_angles)):
        ...
        camera_wrapper.set_camera_fov(fov=95)
        ...
        # breakpoint()
        camera_pos_mirage = np.array([0.25,0.4,0.3])
        camera_rot_mirage = np.array([[-0.9018481, -0.0713397,  0.4261228],
        ...
        target_pos = target_pose[-4:-1]
        target_pos += np.array([-0.6, 0.0, 0.912])
        target_quat = T.mat2quat(target_pose[:9].reshape([3, 3]))
