...
if __name__ == "__main__":
    ...
    joint_angles = np.loadtxt("/home/lawrence/xembody_followup/viola_dataset/bowldomain/joint_states.txt")
    ee_poses = np.loadtxt("/home/lawrence/xembody_followup/viola_dataset/bowldomain/ee_states.txt")
    ...
    breakpoint()
    # camera_wrapper.set_camera_pose(pos=camera_pos+np.array([0,0.04,0.02]), quat=camera_quat, offset=np.zeros(3))
    camera_pos = np.array([0.5 , 0.04  , 1.37])
    camera_rot = np.array([[-4.47034836e-08, 7.06147254e-01, -7.08065033e-01],
    ...
    camera_quat_mirage = T.mat2quat(camera_rot)
    print("Original quat:", camera_quat, "Mirage quat:", camera_quat_mirage)
    camera_wrapper.set_camera_pose(pos=camera_pos, quat=camera_quat_mirage, offset=np.zeros(3))
    for _ in range(50):
    ...
    for k in range(len(joint_angles)):
        ...
        target_pose = ee_poses[k].reshape((4, 4)).T
        target_pose[:3, -1] += np.array([-0.6, 0.0, 0.912])
        target_pos, target_quat = T.mat2pose(target_pose)
        target_quat = T.quat_multiply(target_quat, np.array([ 0, 0, -0.7071068, 0.7071068 ]))
        reached = set_robot_joint_positions(env, joint_angles=joint_angles[k])
        ee_pose = compute_eef_pose(env)
        print("Target pose: ", target_pos, target_quat)
        ...
        for i in range(1):
            ...
            for view in ["agentview"]:
                ...
                import cv2
                # cv2.imwrite(f"plateforkdomain/{k*2}_sim.png", cv2.cvtColor(front_rgb_img, cv2.COLOR_RGB2BGR))
