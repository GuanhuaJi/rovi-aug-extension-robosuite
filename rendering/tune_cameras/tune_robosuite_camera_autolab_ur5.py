...
def increase_brightness(img, value=30, mask=None):
...
if __name__ == "__main__":
    ...
    options["robots"] = "UR5e"   # You can choose your desired robot here
    options['gripper_types'] = "Robotiq85Gripper"
    ...
    joint_angles = np.loadtxt("/home/lawrence/xembody_followup/autolab_ur5_dataset/joint_states.txt")
    ee_poses = np.loadtxt("/home/lawrence/xembody_followup/autolab_ur5_dataset/ee_states.txt")
    ...
    for k in range(len(joint_angles)):
        ...
        camera_wrapper.set_camera_fov(fov=100)
        ...
        # breakpoint()
        camera_pos_mirage = np.array([-0.07,0.35,0.3])
        camera_rot_mirage = np.array([[-0.5,  -0.1, 0.6],
                                    [ -0.5,  0.1, -0.3],
                                    [-0.1, -0.9, -0.3]])
        ...
        joint_angle = joint_angles[k]#+ np.random.normal(0, 0.1, 6)
