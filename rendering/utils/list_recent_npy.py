...
CUTOFF   = dt.datetime(2025, 6, 12, 18, 0, tzinfo=PACIFIC)   # 2025-06-13 00:00 PT
...
#     "Sawyer": {
#         "recent": np.array([0, 0, 0]),
#         "old": np.array([0, 0, 0.02]),
#     },
#     "Jaco": {
#         "recent": np.array([0, 0, 0]),
#         "old": np.array([0.02, 0, 0.02]),
#     },
#     "UR5e": {
#         "recent": np.array([0, 0, 0]),
#         "old": np.array([0, 0, 0]),
...
#     },
#     "Jaco": {
#         "recent": np.array([0.18, 0, 0]),
#         "old": np.array([0.18, 0, 0]),
#     },
#     "UR5e": {
#         "recent": np.array([0, 0, 0]),
#         "old": np.array([0, 0, 0]),
...
def main(root: Path):
    for robot in ROBOTS:
        ...
        for p in all_npy:
            ...
            offset = offset_dict[robot]["recent"] if is_recent else offset_dict[robot]["old"]

            out_path = offset_path / p.name
            np.save(out_path, offset)

            status = "recent" if is_recent else "old"
            print(f"  {status:6} → {out_path}")

from dataset_pair_location import dataset_path
if __name__ == "__main__":
    dataset = "austin_sailor"  # Change this to the desired dataset
    main(Path(f"{dataset_path[dataset]}/{dataset}/source_robot_states"))