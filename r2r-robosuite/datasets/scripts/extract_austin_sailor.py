...
def main():
    ...
    for episode_num, episode in enumerate(ds):
        ...
        # Prepare lists to store data for each step
        ee_states_list = []      # End-effector state (16,)
        joint_states_list = []   # 7-dof joint state (7,)
        gripper_states_list = [] # Gripper state (1,) — "breaper state"
        language_instructions_list = [] # language instructions
        ...
        for step_idx, step in enumerate(steps_dataset):
            ...
            # EE state: from "state_ee", shape=(16,)
            ee_state = step["observation"]["state_ee"].numpy()
            # Joint state: from "state_joint", shape=(7,)
            joint_state = step["observation"]["state_joint"].numpy()
            # Gripper state: from "state_gripper", shape=(1,)
