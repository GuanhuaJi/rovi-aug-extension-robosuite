...
def flow_warp(x,
    ...
    grid = torch.stack((grid_x, grid_y), 2).to(x.device).type_as(x)
...
#               align_corners=True):
#     """
#     Warp an image / feature map `x` (N,C,H,W) with optical‑flow `flow`
#     (N,H,W,2) and print debug info each call.
#     """
#     # --------------------------------------------------------------
#     # static call counter for easier tracing
#     # --------------------------------------------------------------
#     if not hasattr(flow_warp, "_call_id"):
#         flow_warp._call_id = 0
#     cid = flow_warp._call_id
#     flow_warp._call_id += 1

#     print(f"\n[flow_warp #{cid}]  x {tuple(x.shape)}  flow {tuple(flow.shape)}  device {x.device}")

#     # --------------------------------------------------------------
#     # Early exit for degenerate maps
#     # --------------------------------------------------------------
#     if x.size(-1) == 1 or x.size(-2) == 1:
#         print(f"[flow_warp #{cid}]  degenerate map → skip warp")
#         return x

#     n, _, h, w = x.size()

#     # --------------------------------------------------------------
#     # Basic sanity on the incoming flow
#     # --------------------------------------------------------------
#     flow_nan  = torch.isnan(flow).any().item()
#     flow_inf  = torch.isinf(flow).any().item()
#     flow_min  = flow.min().item()
#     flow_max  = flow.max().item()
#     print(f"[flow_warp #{cid}]  flow   min {flow_min:.3f}   max {flow_max:.3f}   NaN {flow_nan}   Inf {flow_inf}")
...
#         indexing='ij'
#     )
#     base = torch.stack((grid_x, grid_y), dim=2).unsqueeze(0).repeat(n, 1, 1, 1)

#     vgrid = base + flow      # pixel coordinates before clamping

#     # Any pixel coords already crazy?
#     oob_pix = (
#         (vgrid[..., 0] < 0) | (vgrid[..., 0] > w - 1) |
#         (vgrid[..., 1] < 0) | (vgrid[..., 1] > h - 1)
#     ).any().item()
#     print(f"[flow_warp #{cid}]  pixel grid  OOB before clamp? {oob_pix}")

#     # Clamp to valid pixel window
#     vgrid[..., 0].clamp_(0.0, w - 1.0)
#     vgrid[..., 1].clamp_(0.0, h - 1.0)

#     # --------------------------------------------------------------
#     # Normalise to [-1, 1]
#     # --------------------------------------------------------------
#     vgrid_x = 2.0 * vgrid[..., 0] / (w - 1 if w > 1 else 1) - 1.0
#     vgrid_y = 2.0 * vgrid[..., 1] / (h - 1 if h > 1 else 1) - 1.0
#     vgrid   = torch.stack((vgrid_x, vgrid_y), dim=3)

#     oob_norm = ((vgrid < -1) | (vgrid > 1)).any().item()
#     print(f"[flow_warp #{cid}]  norm grid  OOB before final clamp? {oob_norm}")

#     # Tiny guard against fp round‑off
#     vgrid.clamp_(-1.0 + 1e-6, 1.0 - 1e-6)
